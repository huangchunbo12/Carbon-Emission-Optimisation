% ===== Run WOA multiple times and write each iteration process to Excel =====
close all; clear; clc
% ===== Basic parameters =====
N = 30;
Function_name = 'F29';
...
% WOA: N evaluations for initialization; ~N evaluations per iteration -> N + N*Max_iter <= MaxFEs
Max_iter = max(1, floor((MaxFEs - N) / N));
nRuns   = 10;
% (Optional) WOA parameters; leave empty to use defaults (b=1.0)
params = struct('b', 1.0);
% ===== Excel file name (with timestamp) =====
ts = datestr(now,'yyyymmdd_HHMMSS');
xlsxFile = sprintf('WOA_%s_%s.xlsx', Function_name, ts);
...
for r = 1:nRuns
    ...
    [woa_score, woa_pos, woa_curve] = WOA(N, Max_iter, lb, ub, dim, fobj, params);
    curves{r} = woa_curve(:)';   % Store the curve
    % Write the current iteration process to a separate sheet
    T = table((1:numel(woa_curve))', woa_curve(:), ...
    ...
    % Summary information
    summaryTbl.Run(r)        = r;
    summaryTbl.BestScore(r)  = woa_score;
    summaryTbl.Iterations(r) = numel(woa_curve);
    bestPosAll(r,:)          = woa_pos;
    fprintf('Run %02d | BestScore = %.6g | Iterations = %d\n', ...
        r, woa_score, numel(woa_curve));
end
...
% (Optional) Record WOA parameters
paramNames = fieldnames(params);
paramVals  = struct2cell(params);
paramTbl   = table(paramNames, paramVals, 'VariableNames', {'Param','Value'});
writetable(paramTbl, xlsxFile, 'Sheet', 'WOA_Params', 'WriteMode', 'overwritesheet');
% ===== Visualization: function surface + overlay all WOA convergence curves =====
...
title([Function_name, '  (WOA, ', num2str(nRuns), ' runs)']);
